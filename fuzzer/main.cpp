...
void save_to_file(const char *raw_error_output, long CurrentInput)
{
    ...
    if (grep_content != "")
    {
        ...
        std::string command = "cp inputs/AUTOGEN_" + std::to_string(CurrentInput) + ".cnf ./newDir/AUTOGEN_" + std::to_string(CurrentInput) + ".cnf; echo '----'; ls; echo '----'; pwd; echo '----'; cat test; echo '----'";
